function [Y cost] = sne(X, labels, d, perplexity, eta)
%SNE Implementation of Stochastic Neighbor Embedding
%
%   mappedX = sne(X, labels, no_dims, perplexity)
%
...
for iter=1:max_iter
    ...
    %         axis equal tight
    %     elseif d==3
    %         scatter3(Y(:,1), Y(:,2),Y(:,3), 10,labels, 'filled');
    %         axis equal tight
    %     else
    %         printf('error')
    %     end
    ...
    costs = sum(P .* log((P + eps) ./ (Q + eps)), 2) ./ n;
    cost = sum(costs);
