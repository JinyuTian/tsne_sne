...
%     using the number of the labels to do clustering evaluation
    numComponents_sne=16;%max(label);
    numComponents_tsne=16;%max(label);

%     %using the number of the labels we got from F1-measure to do clusering evaluation
%     [mus_sne,stds_sne]=cluster_validation(Y_sne,label,[30 1 15]);
%     [mus_tsne,std_tsne]=cluster_validation(Y_tsne,label,[30 1 15]);
...
%     [temp,numComponents_sne]=max(mus_sne(3,:));
    ...
    [mus_sne,stds_sne]=cluster_validation(Y_sne,label,[20 1 30]);
    [mus_tsne,std_tsne]=cluster_validation(Y_tsne,label,[20 1 30]);
